...
```{r}
gpa_data |>
  summarise(mean_GPA = mean(GPA), var_GPA = var(GPA), mean_ACT = mean(ACT), var_ACT = var(ACT))
```
...
When the test score increases by 1 point, the point estimate will increase by 0.039 points. When the test score increases by 4 points, the point estimate will increase by 0.039(4) = .156 points.
...
**d.** Provide an interpretation of β ̂_2, the coefficient of weight.
